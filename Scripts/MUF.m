function MUFVal = MUF(SSR, month, time, transLat, transLon, recLat, recLon, dayTime, sunTime)
%MUF calculation
    if 6 < time && time < 18
        Isday = 'd';
    else
    ...
    function A2Return = A2(SSR)
        A2Return = 1.3022 - (0.00156)*SSR;
    end
    ...
    function f0F2Return = f0F2(SSR)
        f0F2Return = sqrt(2+(0.814*SSR+22.23)*sqrt(0.5));
    end

    MUFVal = A2(SSR)*A3(month)*A4(Isday,time,sunTime)*M(transLat, transLon, recLat, recLon)*f0F2(SSR);
end
